...
export const InterceptorInner : React.FC<InterceptorInnerProps> = ({interceptor, showBall}) => {
    ...
    const renderBall = () => {
        if (showBall && ballColor !== undefined) {
            return <svg className='ball' height="16" width="16">
                        <circle className='ball-circle' cx="8" cy="8" r="7" stroke="black" strokeWidth="1" fill={ballColor === TumblerBallColor.Blue ? "blue" : "red"} />
                    </svg>
        }
    }
...
export const Interceptor = (props: InterceptorProps) => {
    return (
        ...
        {(appState) => (
            ...
            showBall={appState.appStatus === AppStatus.Idle}
            ></InterceptorInner>
        )}
        </AppContext.Consumer>
    );
